...
fn try_main() -> Result<()> {
    ...
    run_eval(&args.data_dir, &args.eval_dir, args.truth.as_ref().map(|p| p.as_path()), args.specs()?)
}
...
fn run_eval(
    ...
    if !data_dir.exists() {
        bail!("data directory {} does not exist; please use imdb-rename to create it", data_dir.display());
    }
...
impl Args {
    ...
    fn from_matches(matches: &clap::ArgMatches) -> Result<Args> {
        ...
        let similarities = parse_many_lossy(
            matches,
            "sim",
            vec![Similarity::None, Similarity::Levenshtein, Similarity::Jaro, Similarity::JaroWinkler],
        )?;
    ...
    App::new("imdb-rename")
        ...
        .arg(
            Arg::with_name("dry-run")
                .long("dry-run")
                .help("Show the evaluations that would be run and then exit without running them."),
        )
        .arg(
        ...
        .arg(Arg::with_name("ngram-size").long("ngram-size").takes_value(true).multiple(true).number_of_values(1).help(
            "Set the ngram size on which to perform an evaluation. An evaluation will be performed for each ngram size. If no \
             ngram size is given, a default of 3 is used.",
        ))
        .arg(
            Arg::with_name("ngram-type")
                ...
                .help(
                    "Set the ngram type on which to perform an evaluation. An evaluation will be performed for each ngram type. \
                     If no ngram type is given, it defaults to 'window'.",
                ),
        )
        .arg(Arg::with_name("result-size").long("result-size").takes_value(true).multiple(true).number_of_values(1).help(
            "Set the result size on which to perform an evaluation. An evaluation will be performed for each result size. If no \
             result size is given, a default of 30 is used.",
        ))
        .arg(
            Arg::with_name("scorer")
                ...
                .help(
                    "Set the name scorer function to use. An evaluation is performed for each name function given. By default, \
        ...
        .arg(
            Arg::with_name("sim")
                ...
                .help(
                    "Set the similarity ranker function to use. An evaluation is performed for each ranker function given. By \
                     default, all ranker functions are used, including 'none'.",
                ),
        )
        .arg(
        ...
        .arg(Arg::with_name("truth").long("truth").takes_value(true).help(
            "A file path containing evaluation truth data. By default, an evaluation uses truth data embedded in imdb-rename.",
        ))
}
